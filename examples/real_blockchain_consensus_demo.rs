...
// 导入我们的模块
use multi_agent_oracle::solana::real_consensus_deployer::{
    RealSolanaDeployer, AgentConsensusResult, AgentGraphData, 
...
/// 真实区块链共识管理器
pub struct RealBlockchainConsensusManager {
    /// 真实Solana部署器
    pub solana_deployer: RealSolanaDeployer,
    /// 任务ID生成器
    pub task_counter: u64,
}

impl RealBlockchainConsensusManager {
    /// 创建新的真实区块链共识管理器
    pub fn new() -> Result<Self> {
        let solana_deployer = RealSolanaDeployer::new(
    ...
    /// 执行完整的真实智能体共识上链流程
    pub async fn execute_real_consensus_on_chain(
        &mut self,
        scenario: &str,
        intervention: &str,
        agents: Vec<SimpleAgent>,
    ) -> Result<String> {
        println!("🚀 启动真实智能体共识上链流程");
        println!("==========================================");
        println!("📋 场景: {}", scenario);
        ...
        // 0. 检查网络连接
        self.solana_deployer.get_network_info()?;
        self.solana_deployer.get_wallet_balance()?;
    ...
    fn generate_task_id(&mut self) -> String {
        ...
        let timestamp = SystemTime::now()
            ...
            .unwrap()
            .as_secs();
        format!("real_consensus_{}_{}", timestamp, self.task_counter)
    }
...
/// 主函数 - 演示完整的真实智能体共识上链流程
#[tokio::main]
pub async fn main() -> Result<()> {
    run_real_blockchain_consensus_demo().await
}

/// 运行真实区块链共识演示
pub async fn run_real_blockchain_consensus_demo() -> Result<()> {
    println!("🚀 启动真实智能体共识上链演示");
    println!("==========================================");
    
    // 创建区块链共识管理器
    let mut manager = RealBlockchainConsensusManager::new()?;
    ...
    // 执行完整的真实共识上链流程
    match manager.execute_real_consensus_on_chain(scenario, intervention, agents).await {
        Ok(transaction_hash) => {
            println!("\n🎉 真实智能体共识上链演示完成!");
            println!("==========================================");
            println!("🔒 安全验证: ✅ 通过");
...
mod tests {
    ...
    #[tokio::test]
    async fn test_real_blockchain_consensus() {
        // 这个测试需要真实的Solana网络连接
        if std::env::var("CI").is_ok() {
            return;
        }
        
        let result = run_real_blockchain_consensus_demo().await;
