...
// 导入我们的模块
use multi_agent_oracle::solana::simple_solana_deployer::{
    SimpleSolanaDeployer, AgentConsensusResult, AgentGraphData, 
...
/// 简化真实区块链共识管理器
pub struct SimpleBlockchainConsensusManager {
    /// 简化Solana部署器
    pub solana_deployer: SimpleSolanaDeployer,
    /// 任务ID生成器
    pub task_counter: u64,
}

impl SimpleBlockchainConsensusManager {
    /// 创建新的简化区块链共识管理器
    pub fn new() -> Self {
        let solana_deployer = SimpleSolanaDeployer::new(
    ...
    /// 执行简化的真实智能体共识上链流程
    pub async fn execute_simple_real_consensus_on_chain(
        &mut self,
        scenario: &str,
        intervention: &str,
        agents: Vec<SimpleAgent>,
    ) -> Result<String> {
        println!("🚀 启动简化真实智能体共识上链流程");
        println!("==========================================");
        println!("📋 场景: {}", scenario);
        ...
        self.solana_deployer.get_wallet_balance().await?;
    ...
    fn generate_task_id(&mut self) -> String {
        ...
        let timestamp = SystemTime::now()
            ...
            .unwrap()
            .as_secs();
        format!("simple_real_consensus_{}_{}", timestamp, self.task_counter)
    }
    ...
    fn display_detailed_results(&self, consensus_data: &AgentConsensusResult, tx_result: &SolanaTransactionResult) {
        ...
        if let Some(height) = tx_result.block_height {
            println!("📦 区块高度: {}", height);
        }
        println!("📅 时间戳: {}", consensus_data.timestamp);
        println!("🔒 交易状态: {:?}", tx_result.status);
...
/// 主函数 - 演示完整的简化真实智能体共识上链流程
#[tokio::main]
pub async fn main() -> Result<()> {
    run_simple_real_blockchain_consensus_demo().await
}

/// 运行简化真实区块链共识演示
pub async fn run_simple_real_blockchain_consensus_demo() -> Result<()> {
    println!("🚀 启动简化真实智能体共识上链演示");
    println!("==========================================");
    
    // 创建区块链共识管理器
    let mut manager = SimpleBlockchainConsensusManager::new();
    ...
    // 执行完整的简化真实共识上链流程
    match manager.execute_simple_real_consensus_on_chain(scenario, intervention, agents).await {
        Ok(transaction_hash) => {
            println!("\n🎉 简化真实智能体共识上链演示完成!");
            println!("==========================================");
            println!("🔒 安全验证: ✅ 通过");
            ...
            println!("🤝 智能体协作: 已完成");
            println!("⚡ 验证速度: <20秒");
            println!("🌐 真实Solana交易: {}", transaction_hash);
            println!("📋 区块链浏览器: https://solscan.io/tx/{}", transaction_hash);
            println!("💡 注意: 这是基于真实RPC连接的简化实现");
        }
        Err(e) => {
            println!("\n❌ 简化真实区块链部署失败: {}", e);
            println!("💡 请确保:");
            println!("   1. Solana本地验证器正在运行: solana-test-validator");
            println!("   2. RPC地址正确配置: http://127.0.0.1:8899");
            println!("   3. 网络连接正常");
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_simple_real_blockchain_consensus() {
        // 这个测试需要真实的Solana网络连接
        if std::env::var("CI").is_ok() {
            return;
        }
        
        let result = run_simple_real_blockchain_consensus_demo().await;
        // 在没有真实网络的情况下，我们期望得到错误
